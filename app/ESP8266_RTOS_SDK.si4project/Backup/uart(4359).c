...
{
	xIotCallback_t * callback=(xIotCallback_t*)pvParameters;
    ...
    for (;;) {
        ...
        xEventGroupWaitBits(callback->handler->wifi_event_group, callback->handler->CONNECTED_BIT, false, true, portMAX_DELAY);
        ...
        if ((rc = MQTTSubscribe(&client, CONFIG_MQTT_SUB_TOPIC, CONFIG_DEFAULT_MQTT_SUB_QOS, callback->pvParameters)) != 0) {
...
{
	xIotCallback_t * callback=(xIotCallback_t *)malloc(sizeof(xIotCallback_t));
	memset(callback,0x00,sizeof(xIotCallback_t));
	callback->handler=handler;
	callback->pvParameters=pvParameters;

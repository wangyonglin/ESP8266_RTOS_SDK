...
static esp_err_t wifi_event_handler(void *ctx, system_event_t *event)
{
	...
	xTaskFramework_t* framework=(xTaskFramework_t*)ctx;
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
		...
		xTaskCreate(framework->pvTaskCode, "xTaskFramework_t", framework->usStackDepth, NULL, 8,NULL);
        break;
...
{
	xTaskFramework_t *framework=NULL;
	 if(framework==NULL){
		framework=(xTaskFramework_t *)malloc(sizeof(xTaskFramework_t));
		memset(framework,0, sizeof(xTaskFramework_t));
	 }
		
	framework->pvTaskCode=pvTaskCode;
	framework->usStackDepth=usStackDepth;
    tcpip_adapter_init();
    mwht.wifi_event_group = xEventGroupCreate();
	mwht.CONNECTED_BIT=BIT0;
	mwht.ESPTOUCH_DONE_BIT=BIT1;	  
    ESP_ERROR_CHECK( esp_event_loop_init(wifi_event_handler,framework));
